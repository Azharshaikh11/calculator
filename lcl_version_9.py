...
def calculate_packing_charges(cft: float) -> float:
    ...
    for key, value in charges.items():
        if isinstance(key, str):
            ...
            if range_values:
                cfts.append(range_values)
                if 'rate' in value:
                    rates.append(value['rate'])

    packing_charges = []
    for i in range(len(cfts)):
        if i == 0:
            packing_charges.append(cfts[i][1])
        else:
            last_value = cfts[i][1] - cfts[i-1][1]
            packing_charges.append(last_value)

    y = [0] * 5
    cft_temp = cft

    for i in range(5):
        if i < len(packing_charges):
            if cft_temp - packing_charges[i] > packing_charges[i]:
                y[i] = packing_charges[i]
                cft_temp -= packing_charges[i]
            elif cft_temp > packing_charges[i]:
                y[i] = packing_charges[i]
                cft_temp -= y[i]
            else:
                y[i] = cft_temp
    ...
    if cft < charges.get('minimumUptoCft', 0):
        pc = max(pc, charges.get('minimumPacking', 0))
