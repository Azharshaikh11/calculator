...
def calculate_packing_charges(cft: float) -> float:
    ...
    # Define slab structure based on new charges format
    slabs = [
        (0, 50, 'cft0to50'),
        (51, 100, 'cft51to100'),
        (101, 200, 'cft101to200'),
        (201, 350, 'cft201to350'),
        (351, 600, 'cft351to600'),
        (601, 2000, 'cft601to2000')
    ]
    
    for slab in slabs:
        min_cft, max_cft, key = slab
        if remaining_cft <= 0:
            break
            
        # Calculate cft in this slab
        slab_range = max_cft - min_cft + 1
        cft_in_slab = min(remaining_cft, slab_range)
        
        # Add to packing cost
        pc += cft_in_slab * charges[key]['rate']
        remaining_cft -= cft_in_slab
